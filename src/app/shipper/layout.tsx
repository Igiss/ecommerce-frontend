...
import { LayoutDashboard, MapPin, ArrowLeft, Loader2, LogOut, Shield, Package } from 'lucide-react'
...
export default function ShipperLayout({ children }: ShipperLayoutProps) {
  ...
  useEffect(() => {
    // role is 'shipper'
    if (mounted && initialized && (!user || user.role !== 'shipper')) {
      router.push('/')
    }
    ...
    return (
      <div className="flex min-h-screen items-center justify-center bg-stone-50 px-4">
        <div className="w-full max-w-md text-center rounded-2xl border border-stone-200 bg-white p-8 shadow-xl">
          ...
          <p className="text-xs text-stone-500 mt-2">Bạn không có quyền truy cập vào cổng thông tin Shipper.</p>
  ...
  const sidebarLinks = [
    { href: '/shipper/dashboard', label: 'Quản lý giao hàng', icon: LayoutDashboard },
    { href: '/shipper/orders', label: 'Danh sách Đơn giao', icon: Package },
  ]
    ...
    <div className="flex min-h-screen bg-stone-100/60">
      ...
      <aside className="w-64 bg-stone-900 text-stone-300 flex flex-col shrink-0">
        ...
        <div className="h-16 flex items-center px-6 border-b border-stone-800">
          <Link href="/" className="flex items-center gap-2">
            <span className="text-lg font-extrabold tracking-tight text-white">
              Cup<span className="text-amber-500">Shop</span> <span className="text-[10px] bg-amber-500/25 text-amber-500 px-1.5 py-0.5 rounded-full font-bold ml-1 uppercase">Shipper</span>
            </span>
          </Link>
        </div>
        ...
        <div className="p-4 border-t border-stone-850 space-y-2">
          <div className="px-3 py-3 rounded-lg bg-stone-800/50 mb-2 flex items-center gap-3">
            <div className="h-10 w-10 bg-amber-800 rounded-full flex items-center justify-center shrink-0">
              <span className="text-white font-bold text-sm">{(user.fullName || 'S')[0]}</span>
            </div>
