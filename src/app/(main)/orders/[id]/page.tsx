...
export default function OrderDetailPage({ params }: OrderDetailPageProps) {
  ...
  if (error || !order) {
    return (
      <div className="mx-auto max-w-2xl px-4 py-16 text-center">
        ...
        <Link
          href="/profile/orders"
          className="mt-6 inline-flex items-center gap-2 rounded-xl bg-amber-800 hover:bg-amber-900 text-white px-5 py-2 text-sm font-bold shadow-md transition-colors"
        >
          <ChevronLeft className="h-4 w-4" /> Quay lại đơn mua của tôi
        </Link>
      </div>
    )
  }
  ...
  return (
    <div className="mx-auto max-w-4xl px-4 py-10 sm:px-6 lg:px-8">
      {/* Back button */}
      <Link href="/profile/orders" className="inline-flex items-center gap-1 text-sm text-stone-500 hover:text-amber-800 transition-colors mb-6">
        <ChevronLeft className="h-4 w-4" />
        Quay lại đơn mua của tôi
      </Link>
      ...
      {/* Bill summary */}
      <div className="rounded-2xl border border-stone-200 bg-white p-6 shadow-xs w-full">
